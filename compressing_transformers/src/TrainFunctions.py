...
class TrainFunctions:
    ...
    def __init__(self, args: dict, other_settings: dict, path_to_database: str, experiment_name: str):
        ...
        total_number_of_GPUs = self.distributed_trainer.world_size

        self.args["seq_length"] = transformer_config["seq_length"]  # Store sequence length (=context window) for dataset preparation
        self.args["batch_size"]  = transformer_config["batch_size_per_gpu"]*total_number_of_GPUs
    ...
    def calculate_some_metrics(self, ddp_model: DDP, dataloaders: list, logs: dict):
        ...
        if self.other_settings["calculate_train_loss"]:
            ...
            train_loss = loss_over_dataset(ddp_model, dataloader_train, self.args, self.distributed_trainer, debug=self.other_settings["debug"])
            if rank == 0:
        ...
        if self.other_settings["calculate_test_loss"]:
            ...
            test_loss = loss_over_dataset(ddp_model, dataloader_test, self.args, self.distributed_trainer, debug=self.other_settings["debug"])
