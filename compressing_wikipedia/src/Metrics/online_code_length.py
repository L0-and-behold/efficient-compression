...
def online_code_length(run_df: RunsCSV, args: dict) -> float:
    """
    Calculate the code length of the online setting. In the online setting, the transformer is trained on a (small) batch of data. The data is then compressed using the paritally trained model.
    The coding length of transmitting a dataset in this setting can be calculated from the loss (which is crossentropy (log)) since it directly relates to the coding length (which is log2 based).    
    """
