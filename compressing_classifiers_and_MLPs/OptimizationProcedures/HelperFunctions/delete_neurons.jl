...
"""
    delete_neurons_lux!(state::Lux.Training.TrainState, loss_fun, astate=nothing; reassign=false)
    ...
    Returns tstate (containing all model, optimizer and network parameters), loss_fun (the loss function) and astate (another TrainState object like tstate, which is only used if alternating descent and ascent updates are performed for minimax objectives).
"""
function delete_neurons_lux!(state::Lux.Training.TrainState, loss_fun, astate=nothing; reassign=false)
    ...
    # biases_u = []
    out_dims = 0
    if tstate.model.name == "ascent model" # TODO
        activation_of_previous_layer = tstate.model.layers.layer_1.layer.activation
    elseif typeof(tstate.model.layers[1]) <: FlattenLayer # !haskey(tstate.model.layers.layer_1, :activation)
    ...
    for (l_name, l) in pairs(tps)
        ...
        if i>1 # this uses values from the previous iteration to modify the present layer
            if tstate.model.name == "ascent model"
                @reset tstate.model.layers[Symbol(l_name)].layer.in_dims = out_dims # the modified out_dims of previous layer are also used as in_dims of the present layer
                @reset astate.model.layers[Symbol(l_name)].layer.in_dims = out_dims
            else
                @reset tstate.model.layers[Symbol(l_name)].in_dims = out_dims
            end
            if tstate.model.name == "ascent model" || (tstate.model.name == "PMMP model" && !haskey(tstate.parameters, :pp))
                tps[Symbol(l_name)].bias_w .+= tps[Symbol(l_name)].weight_w[:, biases_w[1]] * activation_of_previous_layer.(biases_w[2])
            end
            ...
            if tstate.model.name == "ascent model"
                ...
                @reset astate.parameters[Symbol(l_name)].u_w = astate.parameters[Symbol(l_name)].u_w[:,column_mask]
                @reset tstate.states[Symbol(l_name)].frozen_params.u_w = astate.parameters[Symbol(l_name)].u_w
            end
            ...
            if !isnothing(astate)
                @reset astate.optimizer_state = update_optimizer_state!(astate.optimizer_state, l_name, column_mask, 2)
            end
            if haskey(tstate.parameters, :pw) && haskey(tstate.parameters, :pp)
        ...
        if tstate.model.name == "ascent model" # || tstate.model.name == "PMMP model"
            if haskey(tstate.states, :mask)
                biases_w = (bias_inds, tps[Symbol(l_name)].bias_w[bias_inds] .* tstate.states.mask[Symbol(l_name)].bias_w[bias_inds])
            else
                biases_w = (bias_inds, tps[Symbol(l_name)].bias_w[bias_inds])
            end
        end
        if haskey(tstate.parameters, :pw)
        ...
        if tstate.model.name == "ascent model"
            @reset tps[Symbol(l_name)].weight_w = tps[Symbol(l_name)].weight[row_mask,:]
            @reset tps[Symbol(l_name)].bias_w = tps[Symbol(l_name)].bias[row_mask]
            
            @reset tps[Symbol(l_name)].weight_p = tps[Symbol(l_name)].weight[row_mask,:]
            @reset tps[Symbol(l_name)].bias_p = tps[Symbol(l_name)].bias[row_mask]
            
            @reset astate.parameters[Symbol(l_name)].u_w = astate.parameters[Symbol(l_name)].u_w[row_mask,:]
            @reset astate.parameters[Symbol(l_name)].u_b = astate.parameters[Symbol(l_name)].u_b[row_mask]
            
            @reset tstate.states[Symbol(l_name)].frozen_params.u_w = astate.parameters[Symbol(l_name)].u_w
            @reset tstate.states[Symbol(l_name)].frozen_params.u_b = astate.parameters[Symbol(l_name)].u_b

            @reset tstate.model.layers[Symbol(l_name)].layer.out_dims = out_dims
            @reset astate.model.layers[Symbol(l_name)].layer.out_dims = out_dims
            activation_of_previous_layer = tstate.model.layers[Symbol(l_name)].layer.activation
        else
        ...
        if !isnothing(astate)
            @reset astate.optimizer_state = update_optimizer_state!(astate.optimizer_state, l_name, row_mask, 1)
        end
        if haskey(tstate.parameters, :pw) && haskey(tstate.parameters, :pp)
    ...
    if tstate.model.name == "ascent model" # transfer params from tstate to astate
        for (l_name, l) in pairs(tstate.parameters)
            for (w_name, w) in pairs(l)
                @reset astate.states[Symbol(l_name)].frozen_params[Symbol(w_name)] = w
            end
        end
    end
    return tstate, loss_fun, astate 
end
