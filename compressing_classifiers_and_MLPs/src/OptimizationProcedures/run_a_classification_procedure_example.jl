...
# # Convert to GB
# println("Used: ", used / 1024^3, " GB")
# println("Total: ", total / 1024^3, " GB")
...
# println("Total: ", total_mem / 1024^3, " GB")
# println("Free: ", free_mem / 1024^3, " GB")
# println("Used: ", used_mem / 1024^3, " GB")
# println("Percentage used: ", 100 * used_mem / total_mem, "%")

# varinfo(sortby=:size, descending=true)
# varinfs = varinfo();
# df = varinfo(sortby=:size, imported=false);
...
# vars = names(Main, all=false, imported=false)
# data = [(name=string(v), size_mb=Base.summarysize(getfield(Main, v))/1024^2) 
#         for v in vars if isdefined(Main, v)]
# df = DataFrame(data)
# sort!(df, :size_mb, rev=true)
# df.size_mb = round.(df.size_mb, digits=2)
# sum(df.size_mb)
# sum(df.size_mb[2:end])
